...
/** Handles PAGE_POST_APPROVED → /topic/page/{pageId}/posts */
@Slf4j
@Component
@RequiredArgsConstructor
public class PagePostApprovedEventHandler implements RedisEventHandler {
    ...
    @Override
    public String getSupportedEventType() { return DomainEventType.PAGE_POST_APPROVED.toString(); }

    @Override
    public void handle(Object eventPayload, Set<Long> targetMemberIds) {
        PagePostEvent event = (PagePostEvent) eventPayload;
        log.info("Broadcasting PAGE_POST_APPROVED for page {} post {}", event.getPageId(), event.getPostId());
